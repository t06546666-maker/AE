  ...
  return <>
    ...
    <section className="panel">
      ...
      {orders.isPending ? <LoadingState /> : orders.isError ? <ErrorState error={orders.error} retry={() => orders.refetch()} /> : !orders.data?.orders.length ? <EmptyState>No WhatsApp customer orders yet.</EmptyState> : <>
        <div className="table-scroll"><table className="customer-orders-table"><thead><tr><th>Request</th><th>Customer</th>{user.role === 'admin' ? <th>Merchant</th> : null}<th>Items</th><th>Amount</th><th>Status</th><th>Received</th>{user.role === 'merchant' ? <th>Update</th> : null}</tr></thead><tbody>
          {orders.data.orders.map((order) => <tr key={order.id}><td><strong className="order-number">{order.requestNo}</strong></td><td><strong>{order.customer}</strong><small>{order.customerPhone}</small></td>{user.role === 'admin' ? <td>{order.merchant}</td> : null}<td className="customer-order-items">{order.items.map((item) => <span key={item.id}>{item.quantity} x {item.name}{item.type === 'custom' ? ' (custom)' : ''}</span>)}</td><td className="amount-column">{order.total === null ? <span className="tag muted">Quote needed</span> : <strong>{formatCurrency(order.total)}</strong>}</td><td><span className={`tag ${order.status === 'completed' || order.status === 'accepted' ? 'success' : order.status === 'rejected' || order.status === 'cancelled' ? 'danger' : 'info'}`}>{order.status}</span></td><td>{formatDate(order.createdAt)}</td>{user.role === 'merchant' ? <td><select aria-label={`Update ${order.requestNo}`} value={order.status} disabled={updateStatus.isPending} onChange={(event) => updateStatus.mutate({ order, nextStatus: event.target.value as CustomerOrderStatus })}>{statuses.map((value) => <option key={value} value={value}>{value}</option>)}</select></td> : null}</tr>)}
        </tbody></table></div>
