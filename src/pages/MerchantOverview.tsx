...
export function MerchantOverview({ user }: { user: UserProfile }) {
  ...
  return <div className="merchant-overview">
    <header className="mo-heading"><div><span className="mo-eyebrow">YOUR BUSINESS, CLOSER</span><h1>{view === 'overview' ? `Hello, ${user.full_name || 'there'}` : view === 'customers' ? 'Your customers' : view === 'insights' ? 'Customer insights' : 'Business reports'}</h1><p>{view === 'customers' ? 'Every relationship starts with knowing your customer.' : 'Know your customers. Keep them coming back.'}</p></div><div className="mo-heading-actions"><button className="mo-close" aria-label="Refresh business data" disabled={query.isFetching} onClick={() => query.refetch()}><RefreshCw size={18} className={query.isFetching ? 'mo-spinning' : ''} /></button>{view !== 'customers' && <label className="mo-month"><CalendarDays size={17} /><input aria-label="Reporting month" type="month" min="2000-01" max={currentMonth} value={month} onChange={e => { if (e.target.value) { const next = new URLSearchParams(params); next.set('month', e.target.value); setParams(next, { replace: true }); } }} /></label>}</div></header>
    ...
    {view === 'overview' && <>
      ...
      <div className="mo-main-actions"><Link to="/add-customer" className="mo-main-action add"><span className="mo-scan-icon"><UserPlus size={22} /></span><span><strong>Add customer</strong><small>Create a customer and send their QR</small></span><ChevronRight size={20} /></Link><Link to="/rewards" className="mo-main-action scan"><span className="mo-scan-icon"><BarChart3 size={22} /></span><span><strong>Scan customer QR</strong><small>Record a purchase or redeem points</small></span><ChevronRight size={20} /></Link></div>
