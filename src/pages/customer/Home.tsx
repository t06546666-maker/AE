  ...
  return (
    <div className="bg-white min-h-screen text-gray-900 font-sans pb-[100px]">
      ...
      {productListOpen ? <div className="fixed inset-0 z-[1100] flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-4" onMouseDown={event => { if (event.target === event.currentTarget) setProductListOpen(false); }}><div className="max-h-[90vh] w-full max-w-[430px] overflow-y-auto rounded-t-3xl bg-white p-5 shadow-xl sm:rounded-3xl">
        ...
        <div className="mb-5 rounded-2xl bg-gray-50 p-3"><h3 className="mb-2 text-sm font-bold text-gray-900">Sent to merchants</h3>{productHistory?.requests?.length ? <div className="space-y-2">{productHistory.requests.map(request => { const expanded = expandedProductList === request.id; return <button type="button" key={request.id} onClick={() => setExpandedProductList(expanded ? null : request.id)} className="w-full rounded-xl bg-white p-3 text-left text-xs shadow-sm"><div className="flex items-center justify-between gap-2"><span className="font-bold text-gray-900">{request.merchants?.name || 'Merchant'}</span><span className="rounded-full bg-green-100 px-2 py-1 font-semibold capitalize text-green-700">{request.status}</span></div><p className={`mt-1 text-gray-600 ${expanded ? 'whitespace-pre-wrap' : 'line-clamp-2'}`}>{request.product_list || 'Photo product list uploaded'}</p><p className="mt-1 text-gray-400">{new Date(request.created_at).toLocaleDateString()} · {expanded ? 'Tap to collapse' : 'Tap to view full list'}</p></button>; })}</div> : <p className="text-xs text-gray-500">No product lists sent yet.</p>}</div>
